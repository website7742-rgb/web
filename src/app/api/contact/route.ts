...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      return NextResponse.json(
        { success: false, error: 'Too many inquiries submitted. Please wait 1 minute before sending another message.' },
        { status: 429 }
      );
    }

    // 3. HTML Escaping to prevent HTML/XSS injection in emails
    const safeName = escapeHtml(name);
    const safeEmail = escapeHtml(email);
    const safeSubject = escapeHtml(subject);
    const safeMessage = escapeHtml(message);

    // 4. Email Template A: Customer Confirmation Email
    const customerHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #0d0d0d; color: #ffffff; padding: 30px; border-radius: 12px; border: 1px solid #222;">
        ...
        <p style="font-size: 16px; color: #e4e4e7; line-height: 1.6;">Peace <strong>${safeName}</strong>,</p>
        <p style="font-size: 14px; color: #a1a1aa; line-height: 1.6;">We received your inquiry regarding <strong>"${safeSubject}"</strong>. Our executive team will review your message and reach out to you if required.</p>
        ...
        <div style="background-color: #18181b; padding: 20px; border-radius: 8px; border-left: 3px solid #dc2626; margin: 20px 0;">
          ...
          <p style="margin: 0; font-size: 14px; color: #ffffff; font-style: italic;">"${safeMessage}"</p>
        </div>
    ...
    // 5. Email Template B: Admin Notification Email to website7742@gmail.com
    const adminHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 650px; margin: 0 auto; background-color: #0a0a0a; color: #ffffff; padding: 30px; border-radius: 12px; border: 2px solid #dc2626;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
          <tr>
            ...
            <td style="padding: 8px 0; color: #ffffff; font-weight: bold; font-size: 14px;">${safeName}</td>
          </tr>
          <tr>
            <td style="padding: 8px 0; color: #a1a1aa; font-size: 13px;">CUSTOMER EMAIL:</td>
            <td style="padding: 8px 0; color: #ef4444; font-weight: bold; font-size: 14px;"><a href="mailto:${safeEmail}" style="color: #ef4444; text-decoration: none;">${safeEmail}</a></td>
          </tr>
          <tr>
            <td style="padding: 8px 0; color: #a1a1aa; font-size: 13px;">SUBJECT / TOPIC:</td>
            <td style="padding: 8px 0; color: #ffffff; font-weight: bold; font-size: 14px;">${safeSubject}</td>
          </tr>
          <tr>
            <td style="padding: 8px 0; color: #a1a1aa; font-size: 13px;">TIMESTAMP:</td>
            <td style="padding: 8px 0; color: #a1a1aa; font-size: 13px;">${escapeHtml(timestamp)}</td>
          </tr>
          <tr>
            <td style="padding: 8px 0; color: #a1a1aa; font-size: 13px;">CLIENT IP:</td>
            <td style="padding: 8px 0; color: #a1a1aa; font-size: 13px;">${escapeHtml(clientIp)}</td>
          </tr>
        </table>
        ...
        <div style="background-color: #18181b; padding: 20px; border-radius: 8px; border: 1px solid #3f3f46; margin: 20px 0;">
          ...
          <p style="margin: 0; font-size: 14px; color: #f4f4f5; line-height: 1.7; white-space: pre-wrap;">${safeMessage}</p>
        </div>

        <div style="margin-top: 25px; text-align: center;">
          <a href="mailto:${safeEmail}?subject=RE: ${encodeURIComponent(subject)}" style="background-color: #dc2626; color: #ffffff; text-decoration: none; padding: 12px 25px; font-weight: bold; font-size: 13px; border-radius: 6px; display: inline-block;">
            REPLY TO ${safeName.toUpperCase()} NOW
          </a>
        </div>
      </div>
    `;
